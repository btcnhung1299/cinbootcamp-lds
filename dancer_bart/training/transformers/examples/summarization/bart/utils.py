...
class SummarizationDataset(Dataset):
    def __init__(
        ...
        print('Load source encoding {}...'.format(max_source_length))
        self.source = torch.load(open(os.path.join(data_dir, 'dancer_enc_train_source_' + str(max_source_length) + '.pt'), 'rb'))
        print('Load target encoding {}...'.format(max_target_length))
        self.target = torch.load(open(os.path.join(data_dir, 'dancer_enc_train_target_' + str(max_target_length) + '.pt'), 'rb'))
